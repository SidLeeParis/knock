Modules.sidlee = function(){
	...
	function feedback(intensity){
		...
		if (_canvas.width > 1400 && intensity < 3){
			intensity = 3;
		}
	...
	function createParticles(){
		...
		for(i = 0; i < _positions.length; i++){
			...
			initialX = (_positions[i].x + (Math.random() - 0.5) * 0.7) * scale + (_canvas.width / 2 - 80 * scale / 2);
			initialY = (_positions[i].y + (Math.random() - 0.5) * 0.7) * scale + (_canvas.height / 2 - 15 * scale / 2);
	...
	function update(){
		_.each(_particles, function(particle, i){
		...
		_.each(_randomParticles, function(particle){
			particle.x += particle.vx;
			particle.y += particle.vy;
